...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!isExecutive) {
      return NextResponse.json({ success: false, message: "You don't have permission to unverify users. Only department executives may unverify members." }, { status: 403 });
    }
    ...
    if (user.department !== userDepartment) {
      return NextResponse.json({ success: false, message: "You can only unverify users in your department" }, { status: 403 });
    }
    ...
    return NextResponse.json({ success: false, message: "Failed to unverify user", error: String(error) }, { status: 500 });
  }
}
